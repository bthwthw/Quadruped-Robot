/*
 * SMC_controller.c  –  patched for ROS2 (include path fixed)
 * Original: MATLAB Coder generated
 */

#include "quadruped_smc_ros2/SMC_controller.h"
#include <math.h>
                    ...
                    const double lambda_val[2], double tau[2])
{
  ...
  if (d > 0.1)       sat_s_idx_0 = 1.0;
  else if (d < -0.1) sat_s_idx_0 = -1.0;
  else               sat_s_idx_0 = d / 0.1;

  d = de[1] + (0.0 * b_q[0] + b_q[1] * lambda_val[1]);
  if (d > 0.1)       sat_s_idx_1 = 1.0;
  else if (d < -0.1) sat_s_idx_1 = -1.0;
  else               sat_s_idx_1 = d / 0.1;
